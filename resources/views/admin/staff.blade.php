    ...
    <div class="space-y-6 fade-in" x-data="staffManagement()">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            @forelse($staff as $member)
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all"
                    x-show="!search || '{{ strtolower($member->full_name ?? '') }}'.includes(search.toLowerCase())">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            <div
                                class="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-100 to-teal-100 flex items-center justify-center">
                                <span class="text-teal-600 text-lg font-bold">
                                    {{ strtoupper(substr($member->full_name ?? 'S', 0, 1)) }}
                                </span>
                            </div>
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white">{{ $member->full_name ?? 'N/A' }}</p>
                    ...
                    <div class="flex space-x-2 pt-4 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <button @click="confirmDelete({{ $member->id }}, '{{ $member->full_name ?? 'this staff' }}')"
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl" @click.stop>
                ...
                <form :action="editMode ? `/admin/staff/${form.id}` : '{{ route('admin.staff.store') }}'" method="POST"
                    class="p-6 space-y-4">
                    @csrf
                    ...
                    <div class="flex justify-end space-x-3 pt-2">
                        ...
                        <button type="submit"
                            class="px-6 py-2.5 rounded-xl bg-teal-500 text-white font-medium hover:bg-teal-600">
            ...
            <div class="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 text-center"
                ...
                <form :action="`/admin/staff/${deleteTarget.id}`" method="POST">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <button type="submit"
                            class="flex-1 py-2.5 rounded-xl bg-red-500 text-white font-medium hover:bg-red-600">Remove</button>
                    </div>
                </form>
            </div>
        </div>
