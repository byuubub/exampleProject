...
@section('content')
    ...
    <div class="space-y-6 fade-in" x-data="hospitalManagement()">
                ...
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700/50 text-left">
                        <th
                            ...
                            Hospital</th>
                        <th
                            class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">
                            Location</th>
                        <th
                            class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden lg:table-cell">
                            Contact</th>
                        <th
                            class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            Status</th>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    <div
                                        class="w-10 h-10 rounded-xl bg-teal-100 dark:bg-teal-900/30 flex items-center justify-center flex-shrink-0">
                                        <i class="fa-solid fa-hospital text-teal-600"></i>
                                    </div>
                                    <div>
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $hospital->name }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400 md:hidden">{{ $hospital->city }}
                                        </p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 hidden md:table-cell">
                                ...
                                @if ($hospital->address)
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ Str::limit($hospital->address, 40) }}</p>
                                @endif
                            </td>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-xl" @click.stop>
                ...
                <form :action="editMode ? `/admin/hospitals/${form.id}` : '{{ route('admin.hospitals.store') }}'"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Hospital Name
                                *</label>
        ...
        <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 text-center"
                ...
                <p class="text-gray-500 dark:text-gray-400 mb-6">
                    Are you sure you want to delete <strong x-text="deleteTarget.name"></strong>? This action cannot be
                    undone.
                </p>
